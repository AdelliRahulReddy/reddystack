...
assert.match(home, /<img[^>]+src="[^"]*hero-still-dark\.svg"/, 'The hero artwork must be present before JavaScript loads');
assert.match(home, /<img[^>]+src="[^"]*hero-still-light\.svg"/, 'The hero fallback must support the light theme');
